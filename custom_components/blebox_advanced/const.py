...
"""How long a just-written device payload outranks a contradicting poll.

A refresh already in flight when a write lands carries the payload from before
...
Applies to both payloads Home Assistant writes and the device echoes back: the
settings object and the network object.

Lives here rather than next to its users because the coordinator holds the
written value and ``entity`` imports the coordinator, so the constant cannot
